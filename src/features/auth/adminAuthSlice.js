...
const adminAuthSlice = createSlice({
  ...
  reducers: {
    loginAdmin: (state, action) => {
      const admin = action.payload.user || action.payload.admin;
    ...
    updateAdmin: (state, action) => {
      if (state.admin && action.payload) {
        ...
        sessionStorage.setItem(ADMIN_KEYS.info, JSON.stringify(state.admin));
      }
    },
  },
...
export const { loginAdmin, logoutAdmin, setInitialized, updateAdmin } =
