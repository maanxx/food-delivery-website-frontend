  ...
  const handleSubmit = async () => {
    try {
      ...
      if (imageFileList.length > 0 && imageFileList[0]?.originFileObj) {
        fd.append("image", imageFileList[0].originFileObj);
      }

      if (editingProduct) {
        await updateDish(editingProduct.dish_id, fd);
  ...
  const exportItems = [
    {
      ...
      label: (
        <span>
          <FileExcelOutlined style={{ color: "#22a06b", marginRight: 8 }} />
          Xuất Excel (.xlsx)
        </span>
      ),
    ...
    },
    {
      ...
      label: (
        <span>
          <FilePdfOutlined style={{ color: "#e53935", marginRight: 8 }} />
          Xuất PDF (.pdf)
        </span>
      ),
      onClick: () => message.info("Tính năng đang phát triển"),
      ...
      render: (text, record) => (
        <div className={styles.productCell}>
          ...
          <div className={styles.productInfo}>
            ...
            {record.brand && (
              <span className={styles.productBrand}>{record.brand}</span>
            )}
          </div>
        </div>
      ),
        ...
        <div className={styles.actions}>
          ...
          >
            <Tooltip title="Xoá" placement="top">
              <button className={styles.iconBtnDelete}>
                <DeleteOutlined />
              </button>
            </Tooltip>
          </Popconfirm>
      ...
      <div className={styles.pageHeader}>
        <div>
          ...
          <p className={styles.pageSub}>
            Quản lý toàn bộ thực đơn và hàng hoá trong hệ thống
          </p>
        </div>
      ...
      <Row gutter={[16, 16]} className={styles.statsRow}>
        ...
        <Col xs={12} sm={12} md={6}>
          <div className={styles.statCard}>
            ...
            <div className={styles.statBody}>
              <span className={styles.statNum} style={{ color: "#22a06b" }}>
                {stats.active}
              </span>
        ...
        <Col xs={12} sm={12} md={6}>
          <div className={styles.statCard}>
            ...
            <div className={styles.statBody}>
              <span className={styles.statNum} style={{ color: "#6b7280" }}>
                {stats.inactive}
              </span>
            ...
            locale={{
              emptyText: (
                <div className={styles.empty}>
                  <ShoppingOutlined
                    style={{ fontSize: 44, color: "#e0e0e0" }}
                  />
          ...
          <div className={styles.detailBody}>
            <div className={styles.detailImgRow}>
              ...
              <div className={styles.detailMeta}>
                ...
                <div className={styles.detailPrice}>
                  {Number(detailProduct.price).toLocaleString("vi-VN")}₫
                </div>
                {detailProduct.discount_amount > 0 && (
                  <div className={styles.detailDiscount}>
                    Giảm {detailProduct.discount_amount}%
                  </div>
                )}
                <span
                  className={styles.statusBadge}
                  style={{
                    background: (
                      STATUS_COLORS[detailProduct.status] || STATUS_COLORS.draft
                    ).bg,
                    color: (
                      STATUS_COLORS[detailProduct.status] || STATUS_COLORS.draft
                    ).color,
                ...
                >
                  <span
                    ...
                    style={{
                      background: (
                        STATUS_COLORS[detailProduct.status] ||
                        STATUS_COLORS.draft
                      ).color,
                    }}
                  />
            ...
            <div className={styles.detailGrid}>
              <div className={styles.detailItem}>
                <span className={styles.detailLbl}>
              ...
              <div className={styles.detailItem}>
                ...
                <span
                  className={`${styles.stockBadge} ${detailProduct.stock === 0 ? styles.stockOut : detailProduct.stock <= 10 ? styles.stockLow : styles.stockOk}`}
                >
                  {detailProduct.stock}
                </span>
              </div>
              <div className={styles.detailItem}>
                ...
                <span className={styles.detailVal}>
                  {detailProduct.sold_count ?? 0}
                </span>
              </div>
              <div className={styles.detailItem}>
                <span className={styles.detailLbl}>Hiển thị</span>
                <span className={styles.detailVal}>
                  {detailProduct.available ? "Có" : "Không"}
                </span>
              </div>
              {detailProduct.preparation_time && (
                <div className={styles.detailItem}>
                  <span className={styles.detailLbl}>
                    <ClockCircleOutlined /> Chuẩn bị
                  </span>
                  <span className={styles.detailVal}>
                    {detailProduct.preparation_time} phút
                  </span>
                </div>
              )}
              ...
              <div className={styles.detailItem}>
                ...
                <span className={styles.detailVal}>
                  {detailProduct.rating_avg ?? 0} ⭐ (
                  {detailProduct.rating_count ?? 0})
                </span>
              </div>
              <div className={styles.detailItem}>
          ...
          >
            <Upload
              ...
              onChange={({ fileList: newFileList }) => {
                if (newFileList.length > 1) return false;
                setImageFileList(newFileList);
              }}
              onRemove={() => setImageFileList([])}
              defaultFileList={
                editingProduct?.thumbnail_path
                  ? [
                      {
                        uid: "-1",
                        name: "current-image",
                        status: "done",
                        url: editingProduct.thumbnail_path,
                        thumbUrl: editingProduct.thumbnail_path,
                      },
                    ]
                  : []
              }
