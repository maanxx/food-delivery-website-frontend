...
const useWebSocket = () => {
  ...
  useEffect(() => {
    ...
    socket.on("connect", () => {
      ...
      if (currentUserId) {
        socket.emit("join_personal_room", { userId: currentUserId });
        console.log(`Joined personal room: user:${currentUserId}`);
      }
    });
    ...
    socket.on("user_typing", ({ conversationId, userId, username }) => {
      ...
      if (
        !displayName &&
        conversationId &&
        conversationsRef.current?.[conversationId]
      ) {
  ...
  }, [authToken, currentUserId, dispatch]);
